...
│   └── AdaptiveProgressTracker.swift # Live timeline correction from matched output lines
...
- Progress bar: time-based baseline adjusted by `AdaptiveProgressTracker` when matched output lines arrive early/late; late newer milestones may move visual progress backward, shown immediately, updated at 5 fps while idle and immediately after command output
